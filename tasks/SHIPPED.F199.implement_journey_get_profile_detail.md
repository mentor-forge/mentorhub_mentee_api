...
**Summary**
- `GET /api/journey` wired to `JourneyDetailService.get_my_journey_detail`.
- PATCH rejects request body containing `profile` with `403`.
- Route and E2E tests updated; E2E uses seeded Profile/Journey ids (`A000…02`, `A000…03`).

**Testing**
- `pipenv run test`: 63 unit tests pass.
- `pipenv run container`, `pipenv run api`, `pipenv run e2e`: 32 E2E pass (including journey profile assertions).