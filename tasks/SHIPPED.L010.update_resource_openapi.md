...
**Summary of changes**
- Synced `Resource` schema from configurator (includes `archived` in `status` enum plus `url`, `type`, `cost`, `skill_level`, `interests`, `technologies`, `last_verified`, `created`, `saved`).
- Added `ResourceAggregation` and `ResourceDetail` composite schemas.
- Updated `GET /api/resource` to return a JSON array with `offset`/`size` header pagination and archived-filter documentation.
- Updated `GET /api/resource/{resource_id}` to return `ResourceDetail`.
- Fixed path casing to lowercase `/api/resource`; confirmed no Resource `PATCH` operation.

**Testing results**
- YAML parses successfully.
- All component `$ref`s resolve except pre-existing dangling `#/components/schemas/Control` (unrelated).
- `pipenv run lint`: changed Python files formatted; pre-existing unrelated lint failures elsewhere.
