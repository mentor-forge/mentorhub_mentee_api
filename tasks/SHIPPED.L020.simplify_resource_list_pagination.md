...
**Summary of changes**
- `GET /api/resource` reads `offset`/`size` request headers (defaults `0`/`20`, max size `100`).
- Response body is a plain JSON array (no infinite-scroll wrapper).
- Non-admin callers exclude `status: archived`; admin role sees all resources.
- Removed infinite-scroll query parameters from route and service.

**Testing results**
- `pipenv run test`: 149 passed, 25 deselected.
- `pipenv run build`: clean.
- Changed files black-formatted.
