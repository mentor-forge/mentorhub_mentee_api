...
**Summary of changes**
- Synced `Path` schema from configurator (`modules`, `topics`, `resources`, `technologies`, `interests`, `status`, breadcrumbs).
- Added `PathResourceSummary`, `PathTopicDetail`, `PathModuleDetail`, and `PathDetail` schemas.
- Updated `GET /api/path` to return a plain JSON array (no pagination/scroll params).
- Updated `GET /api/path/{path_id}` to return `PathDetail` with enriched nested resources.
- Fixed path casing to lowercase `/api/path`.

**Testing results**
- `pipenv run lint`: clean.
- `pipenv run container`: image built successfully.
- `pipenv run api` + `curl -s http://localhost:8393/docs/openapi.yaml | head`: spec served.
