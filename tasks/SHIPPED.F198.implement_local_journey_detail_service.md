...
**Summary**
- Added `JourneyDetailService.get_my_journey_detail` — delegates journey to `JourneyService.get_my_journey`, loads Profile via `MongoIO.get_document`.
- Missing Profile raises `HTTPNotFound`.

**Testing**
- `pipenv run test`: 4 new service tests pass.
- `pipenv run lint` and `pipenv run build`: pass.