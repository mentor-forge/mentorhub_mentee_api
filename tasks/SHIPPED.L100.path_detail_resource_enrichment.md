...
**Summary of changes**
- Added `ResourceService.get_resources_by_ids` for batch lookup with minimal projection and archived visibility.
- `PathService.get_path` collects nested resource IDs, fetches summaries, and merges into `modules[].topics[].resources[]`.
- Missing or inaccessible (archived for non-admin) resource IDs are omitted from the enriched array.

**Testing results**
- `pipenv run test`: resource batch and path enrichment unit tests pass.
- `pipenv run e2e` (`test/e2e/test_path.py::test_get_path_detail_enriched_resources`): passed against containerized API.
