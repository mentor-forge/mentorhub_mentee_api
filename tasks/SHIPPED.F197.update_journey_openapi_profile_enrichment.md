...
**Summary**
- Added `Profile` schema from MongoDB configurator.
- Added `JourneyDetail` composite (`allOf` Journey + required `profile`).
- Updated `GET /api/journey` to return `JourneyDetail`; mutations remain plain `Journey`.
- Extended `JourneyUpdate` description to forbid `profile`.

**Testing**
- OpenAPI served at `/docs/openapi.yaml` after container rebuild.
