...
class NoteService:
        ...
        Example RBAC implementation:
            if operation == 'update':
                ...
                if 'admin' not in token.get('roles', []):
                    raise HTTPForbidden("Admin role required to update note documents")
            elif operation == 'create':
                # Create requires staff or admin role
                if not any(role in token.get('roles', []) for role in ['staff', 'admin']):
                    raise HTTPForbidden("Staff or admin role required to create note documents")
            elif operation == 'read':
    ...
    def update_note(note_id, data, token, breadcrumb):
        ...
        try:
            ...
            raise HTTPInternalServerError(f"Failed to update note { note_id}")