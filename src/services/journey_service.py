...
# Allowed sort fields for Journey domain
ALLOWED_SORT_FIELDS = [
    "name",
    "description",
    "status",
...
class JourneyService:
    ...
    - Business logic for Journey domain
    """
        ...
        Example RBAC implementation:
            if operation == 'update':
                ...
                if 'admin' not in token.get('roles', []):
                    raise HTTPForbidden("Admin role required to update journey documents")
            elif operation == 'create':
                # Create requires staff or admin role
                if not any(role in token.get('roles', []) for role in ['staff', 'admin']):
                    raise HTTPForbidden("Staff or admin role required to create journey documents")
            elif operation == 'read':
    ...
    def create_journey(data, token, breadcrumb):
        ...
        try:
            ...
            logger.info(
                f"Created journey { journey_id} for user {token.get('user_id')}"
            )
            return journey_id
            ...
            error_msg = str(e)
            logger.error(f"Error creating journey: {error_msg}")
            raise HTTPInternalServerError(f"Failed to create journey: {error_msg}")
    ...
    def get_journeys(
        ...
        """
        Get infinite scroll batch of sorted, filtered journey documents.
        ...
        try:
            ...
            mongo = MongoIO.get_instance()
            config = Config.get_instance()
            collection = mongo.get_collection(config.JOURNEY_COLLECTION_NAME)
            ...
            logger.info(
                f"Retrieved {len(result['items'])} journeys (has_more={result['has_more']}) "
            ...
            logger.error(f"Error retrieving journeys: {str(e)}")
            raise HTTPInternalServerError("Failed to retrieve journeys")

    @staticmethod
    def get_journey(journey_id, token, breadcrumb):
        """
        Retrieve a specific journey document by ID.
        ...
        Returns:
            dict: The journey document

        Raises:
            HTTPNotFound: If journey is not found
        """
        try:
            JourneyService._check_permission(token, "read")
            ...
            logger.info(
                f"Retrieved journey { journey_id} for user {token.get('user_id')}"
            )
            return journey
        except HTTPNotFound:
            raise
        except Exception as e:
            logger.error(f"Error retrieving journey { journey_id}: {str(e)}")
    ...
    def update_journey(journey_id, data, token, breadcrumb):
        ...
        Returns:
            dict: The updated journey document

        Raises:
            HTTPNotFound: If journey is not found
        """
        try:
            JourneyService._check_permission(token, "update")
            ...
            logger.info(
                f"Updated journey { journey_id} for user {token.get('user_id')}"
            )
            return updated
        except (HTTPForbidden, HTTPNotFound):
            raise
        except Exception as e:
            logger.error(f"Error updating journey { journey_id}: {str(e)}")
